...
Julia의 모듈은 코드를 일관성 있는 단위로 구성하는 데 도움이 됩니다. `module NameOfModule ... end`의 문법으로 구분되며 다음과 같은 특징을 가집니다.

1. 모듈은 각각 새로운 전역 범위를 도입하는 별도의 네임스페이스입니다.
   이는 서로 다른 모듈에 있는 한 충돌 없이 다른 함수나 전역 변수에 동일한 이름을 사용할 수 있기 때문에 유용합니다.

2. 모듈은 자세한 네임스페이스 관리를 위한 기능이 있습니다:
   `export`키워드를 사용하여 각각을 이름들의 집합으로 정의하며, `using`과 `import` 키워드를 사용하여 다른 모듈에서 이름을 가져올 수 있습니다. (아래쪽에서 설명).

3. 모듈은 더 빠른 불러오기를 위해 미리 컴파일 될 수 있으며, 런타임 초기화를 위한 코드를 포함할 수 있습니다.

일반적으로 더 큰 Julia 패키지에서는 파일로 구성된 모듈 코드를 볼 수 있습니다. 예를 들어

```julia
module SomeModule

# export, using, import 명령어들은 일반적으로 여기에 작성됩니다; 추가적인 것은 밑에서 이야기 해보겠습니다
...
파일 및 파일의 이름은 대부분 모듈과 관련이 없으며, 모듈은 모듈의 표현식에만 연결됩니다.
모듈당 여러 개의 파일을 가질 수도, 파일당 여러 개의 모듈을 가질 수도 있습니다.
`include`는 소스 파일의 내용을 포함하여 전역 범위에서 평가된 모듈처럼 동작합니다.
이 장에서는 `include`를 사용하지 않기 위한 짧고 간단한 예를 보겠습니다.

일반적으로 모듈 본문을 들여쓰면 전체 파일을 들여쓰게 되므로, 모듈 본문은 들여쓰지 않는 것이 좋습니다.
또한 모듈 이름에 'UpperCamelCase'를 사용하는 것이 일반적이며(Type과 동일), 특히 모듈에 이름이 비슷한 식별자가 포함되어 있는 경우, 이름이 충돌하지 않도록 복수형을 사용합니다.
